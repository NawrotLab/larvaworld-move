...
4. **lib/process** (10%): Data processing pipelines, dataset analysis, metrics computation
5. **lib/param** (7%): Parameter definitions, configuration classes, schemas
6. **lib/sim** (7%): Simulation engines, experiment types, runtime control
7. **lib/util** (6%): Utility functions, helpers, common tools
8. **lib/screen** (4%): Screen rendering, pygame interface, real-time display
9. **dashboards** (1%): Modern web applications (Holoviz/Panel stack)
10. **cli** (1%): Command-line interface tools
